...
function one_file() {
  ...
  # Fix includes 
  perl -wapi\~ -f ${thisdir}/fix-header-locs-ifdh.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
...
function cmake_file() {
  ...
  (( ${fix_whitespace:-0} )) && ed "$F" < fix-whitespace.ed > /dev/null 2>&1
  # Fix CMakeLists.txt 
  perl -wapi\~ -f ${thisdir}/fix-cmake-nutools.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
function cmake_file_2() {
  local F=$1
  printf "$F \n"
  # Optionally fix whitespace
  (( ${fix_whitespace:-0} )) && ed "$F" < fix-whitespace.ed > /dev/null 2>&1
  # Fix CMakeLists.txt 
  perl -wapi\~ -f ${thisdir}/fix-cmake-ifdh.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
function cmake_file_3() {
  local F=$1
  printf "$F \n"
  # Optionally fix whitespace
  (( ${fix_whitespace:-0} )) && ed "$F" < fix-whitespace.ed > /dev/null 2>&1
  # Fix CMakeLists.txt 
  perl -wapi\~ -f ${thisdir}/fix-cmake-genie.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
